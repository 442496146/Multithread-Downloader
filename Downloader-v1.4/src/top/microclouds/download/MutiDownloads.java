...
public class MutiDownloads {
	...
	public void downloadstop() throws IOException {
	...
	public void downloadrestart() throws IOException {
		
		for (int i = 0; i < threadNumber; i++) {
			download(fileDownMessages);
...
class DownThreads extends Thread {
	...
	// private int endPos;// 当前线程下载的结束位置
	...
	public void run() {
		try {
			...
			System.out.println(this.getName() + "文件下载中. . . " + downLengh + "  " + startPos + "  " + "part size : "
					+ currentPartSize);
			while (downLengh < currentPartSize && (readNum = inputStream.read(byteBuffer)) != -1 && !pause) {
				// 写入文件
...
//				// 全部下载完成
//				if (status) {
//					FileWriter saveMessages = new FileWriter("C:\\Users\\Jason\\Desktop\\java线程下载 - 副本\\Downloader\\messages\\config.txt", true);
